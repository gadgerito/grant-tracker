...
# ═════════════════════════════════════════════════════════════════════════════
    ...
    with tab2:
        ...
        milestones_list = sorted(df["milestone"].dropna().unique().tolist()) if not df.empty else []
                ...
                for member in team_df["name"]:
                    ...
                    if not member_delivs.empty:
                        with st.expander(f"📌 {member} ({len(member_delivs)} deliverable(s))"):
                            st.dataframe(
                                member_delivs[["deliverable", "status", "due_date", "milestone"]],
                                use_container_width=True, hide_index=True,
                            )
...
""")
        ...
        with col1:
            ...
            st.download_button(
                "⬇️ Download Deliverables (CSV)",
                data=csv,
                file_name=f"grant_deliverables_{date.today()}.csv",
                mime="text/csv",
            )
        with col2:
            ...
            st.download_button(
                ...
                data=summary_csv,
                file_name=f"grant_summary_{date.today()}.csv",
                mime="text/csv",
            )
